...
public class ContainmentServerTests(params ClientProcesses[] clientProcesses) : WebSocketServerTestBase(clientProcesses)
{
    ...
    /// <summary>
    /// Moves and replaces child nodes from other containment in the same parent.
    /// </summary>
    [Test]
    public void MoveAndReplaceChildFromOtherContainmentInSameParent_Multiple()
    {
        //TODO: emits MoveChildFromOtherContainmentInSameParent instead of MoveAndReplaceChildFromOtherContainmentInSameParent
        _webSocketServer = new WebSocketServer(_lionWebVersion) { Languages = _languages };
        _webSocketServer.StartServer(IpAddress, Port);
    ...
    /// <summary>
    /// Moves a single child node from one containment to another.
    /// </summary>
    [Test]
